...
class TelegramNumberBot:
    ...
    async def is_user_in_cooldown(self, user_id: int) -> tuple[bool, datetime]:
        """Check if user is in 3-hour request cooldown"""
        try:
            if self.supabase:
                result = self.supabase.table('access_request_cooldown').select('*').eq('user_id', user_id).gte('cooldown_until', datetime.now().isoformat()).execute()
                if result.data:
                    cooldown_until = datetime.fromisoformat(result.data[0]['cooldown_until'].replace('Z', '+00:00'))
    ...
    async def create_access_request(self, user_id: int, username: str, first_name: str, last_name: str) -> bool:
        """Create access request and add cooldown"""
        try:
            ...
            result = self.supabase.table('user_access_requests').insert(request_data).execute()
            ...
            if result.data:
                ...
                cooldown_data = {
                    ...
                    'cooldown_until': (datetime.now() + timedelta(hours=3)).isoformat()
                }
                
                # Remove any existing cooldown first
                self.supabase.table('access_request_cooldown').delete().eq('user_id', user_id).execute()
                
                # Add new cooldown
                self.supabase.table('access_request_cooldown').insert(cooldown_data).execute()
    ...
    async def get_pending_requests(self) -> List[Dict]:
        ...
        try:
            if self.supabase:
                result = self.supabase.table('user_access_requests').select('*').eq('status', 'pending').order('requested_at', desc=False).execute()
                return result.data
            return []
        except Exception as e:
    ...
    async def approve_user_request(self, request_id: int, admin_user_id: int, notes: str = "") -> bool:
        ...
        try:
            ...
            # Get the request
            request_result = self.supabase.table('user_access_requests').select('*').eq('id', request_id).execute()
            ...
            self.supabase.table('user_access_requests').update({
                'status': 'approved',
                'admin_response_at': datetime.now().isoformat(),
            ...
            logger.info(f"✅ User {user_id} approved by admin {admin_user_id}")
            return True
            
        except Exception as e:
            logger.error(f"❌ Error approving user request: {e}")
            return False
    
    async def reject_user_request(self, request_id: int, admin_user_id: int, notes: str = "") -> bool:
        ...
        try:
            ...
            logger.info(f"✅ User request {request_id} rejected by admin {admin_user_id}")
            return True
            
        except Exception as e:
            logger.error(f"❌ Error rejecting user request: {e}")
            return False
    
    async def add_approved_user(self, user_id: int, admin_user_id: int, username: str = None, first_name: str = None) -> bool:
        """Manually add approved user (admin command)"""
        try:
            if not self.supabase:
                return False
            ...
            logger.info(f"✅ User {user_id} manually added by admin {admin_user_id}")
            return True
    ...
    async def remove_approved_user(self, user_id: int) -> bool:
        ...
        try:
            ...
            # Deactivate user
            self.supabase.table('approved_users').update({
                'is_active': False,
                'updated_at': datetime.now().isoformat()
            }).eq('user_id', user_id).execute()
            
            # Release any assigned numbers
            if user_id in self.user_sessions:
                session = self.user_sessions[user_id]
                country = session.get('country')
                number = session.get('number')
                if country and number:
                    self.release_number(country, number)
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle /start command with admin approval system"""
        ...
        if not await self.is_user_approved(user_id):
            ...
            in_cooldown, cooldown_until = await self.is_user_in_cooldown(user_id)
            
            if in_cooldown:
                time_remaining = cooldown_until - datetime.now().replace(tzinfo=cooldown_until.tzinfo)
                hours = int(time_remaining.total_seconds() // 3600)
                minutes = int((time_remaining.total_seconds() % 3600) // 60)
                
                await update.message.reply_text(
                    f"⏳ **Request Cooldown Active**\n\n"
                    f"You can request access again in: **{hours}h {minutes}m**\n\n"
                    f"Please wait for the cooldown to expire before making another request.",
                    parse_mode='Markdown'
                )
                return
            ...
            user_has_pending = any(req['user_id'] == user_id for req in pending_requests)
            
            if user_has_pending:
                await update.message.reply_text(
                    "⏳ **Access Request Pending**\n\n"
                    "Your access request is already submitted and waiting for admin approval.\n\n"
                    "Please wait for the admin to review your request.\n\n"
                    "**Note:** You can only make one request every 3 hours."
                )
                return
            
            # Create access request
            success = await self.create_access_request(user_id, username, user_name, last_name)
            ...
            if success:
                ...
                if self.admin_user_id:
                    try:
                        ...
                        logger.info(f"📢 Admin notified about access request from user {user_id}")
                ...
                await update.message.reply_text(
                    "❌ **Request Failed**\n\n"
                    "Failed to submit your access request. Please try again later."
                )
                return
...
"""
    ...
    async def admin_approve(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Admin command to approve user access"""
        user_id = update.effective_user.id
        ...
        if not context.args:
            await update.message.reply_text(
                ...
                "Example: `/approve 123456789 Approved by admin`"
            )
            return
        ...
        try:
            ...
            # Get pending request for this user
            pending_requests = await self.get_pending_requests()
            user_request = None
            ...
            if not user_request:
                await update.message.reply_text(
                    f"❌ No pending request found for user ID {target_user_id}"
                )
                return
            ...
            if success:
                ...
                await update.message.reply_text(
                    ...
                    f"User: {user_request.get('first_name', 'Unknown')}\n"
                    f"Username: {user_request.get('username', 'N/A')}\n\n"
                    f"The user has been notified of approval."
                )
            else:
                await update.message.reply_text(
    ...
    async def admin_reject(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Admin command to reject user access"""
        user_id = update.effective_user.id
        ...
        if not context.args:
            await update.message.reply_text(
                ...
                "Example: `/reject 123456789 Request denied - spam account`"
            )
            return
        ...
        try:
            ...
            if success:
                ...
                await update.message.reply_text(
                    f"❌ **User Request Rejected**\n\n"
                    f"User ID: {target_user_id}\n"
                    f"User: {user_request.get('first_name', 'Unknown')}\n"
                    f"Username: {user_request.get('username', 'N/A')}\n"
                    f"Reason: {reason}\n\n"
                    f"The user has been notified of rejection."
                )
            else:
                await update.message.reply_text(
                    f"❌ Failed to reject user {target_user_id}"
                )
    ...
    async def admin_add_user(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Admin command to manually add approved user"""
        user_id = update.effective_user.id
        ...
        if not context.args:
            await update.message.reply_text(
                "📝 **Admin Add User Usage:**\n\n"
                "`/adduser <user_id> [username] [first_name]`\n\n"
                "Example: `/adduser 123456789 john_doe John`"
            )
            return
        ...
        try:
            ...
            success = await self.add_approved_user(target_user_id, user_id, username, first_name)
            ...
            if success:
                ...
                await update.message.reply_text(
                    f"❌ Failed to add user {target_user_id}"
                )
    ...
    async def admin_remove_user(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Admin command to remove approved user"""
        user_id = update.effective_user.id
        ...
        try:
            ...
            success = await self.remove_approved_user(target_user_id)
            ...
            if success:
                ...
                await update.message.reply_text(
                    f"❌ Failed to remove user {target_user_id}"
                )
    ...
    async def admin_pending(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            for i, req in enumerate(pending_requests[:10], 1):  # Show max 10
                requested_time = datetime.fromisoformat(req['requested_at'].replace('Z', '+00:00'))
                time_ago = datetime.now().replace(tzinfo=requested_time.tzinfo) - requested_time
                ...
                message += f"{i}. **{req.get('first_name', 'Unknown')}**\n"
                message += f"   ID: `{req['user_id']}`\n"
                if req.get('username'):
                    message += f"   Username: @{req['username']}\n"
                message += f"   Requested: {time_str}\n"
                message += f"   Actions: `/approve {req['user_id']}` | `/reject {req['user_id']}`\n\n"
            
            if len(pending_requests) > 10:
                message += f"... and {len(pending_requests) - 10} more requests"
    ...
    async def admin_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if self.supabase:
                ...
                approved_users_result = self.supabase.table('approved_users').select('id', count='exact').eq('is_active', True).execute()
                pending_requests_result = self.supabase.table('user_access_requests').select('id', count='exact').eq('status', 'pending').execute()
                rejected_requests_result = self.supabase.table('user_access_requests').select('id', count='exact').eq('status', 'rejected').execute()
                
                approved_count = approved_users_result.count or 0
                pending_count = pending_requests_result.count or 0
                rejected_count = rejected_requests_result.count or 0
...
💬 Service: {service}"""
    ...
    async def run_bot(self):
        ...
        try:
            ...
            app.add_handler(CommandHandler("approve", self.admin_approve))
            app.add_handler(CommandHandler("reject", self.admin_reject))
