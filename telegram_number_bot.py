...
<<<<<<< HEAD
    async def can_request_access(self, user_id: int) -> bool:
        ...
        try:
            if self.supabase:
                # Check if user has pending request within 3 hours
                result = self.supabase.table('user_approvals').select('*').eq('user_id', user_id).eq('status', 'pending').gte('last_request_at', (datetime.now() - timedelta(hours=3)).isoformat()).execute()
                return len(result.data) == 0
            return True
        except Exception as e:
    ...
    async def request_access(self, user_id: int, user_data: dict):
        """Request access from admin"""
        try:
            if self.supabase:
                # Check if user already has an entry
                existing = self.supabase.table('user_approvals').select('*').eq('user_id', user_id).execute()
                
                if existing.data:
                    # Update existing entry to pending
                    self.supabase.table('user_approvals').update({
                        'status': 'pending',
                    ...
                    # Create new entry
                    self.supabase.table('user_approvals').insert({
                        'user_id': user_id,
    ...
    async def handle_unapproved_user(self, update: Update, context: ContextTypes.DEFAULT_TYPE, user_data: dict):
        """Handle unapproved user access request"""
        user_id = update.effective_user.id
        user_name = update.effective_user.first_name or "User"
        
        # Check if user can request access (3-hour cooldown)
        if not await self.can_request_access(user_id):
            await update.message.reply_text(
                f"⏳ **Access Request Pending**\n\n"
                f"Hi {user_name}! Your access request is already pending.\n\n"
                f"⏰ You can send a new request after 3 hours if not approved.\n"
                f"📞 Contact: @tasktreasur_support for urgent issues."
            )
            return
        
        # Create access request
        request_created = await self.request_access(user_id, user_data)
        
        if request_created:
            # Send confirmation to user
            await update.message.reply_text(
                f"🔐 **Access Request Sent**\n\n"
                f"Hi {user_name}! Your request has been sent to the admin.\n\n"
                f"⏳ **Status:** Pending approval\n"
                f"📅 **Requested:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                f"✅ You'll be notified once approved\n"
                f"❌ If rejected, you can request again after 3 hours\n\n"
                f"📞 **Support:** @tasktreasur_support"
            )
            ...
            await update.message.reply_text(
                f"❌ **Request Failed**\n\n"
                f"Sorry {user_name}, there was an error processing your request.\n"
                f"Please try again later or contact @tasktreasur_support"
            )
    ...
    async def notify_admin_new_request(self, user_id: int, user_data: dict):
        ...
        try:
            if not self.admin_user_id:
                logger.warning("⚠️ No admin configured to receive notifications")
                return
            
            app = Application.builder().token(self.bot_token).build()
            
            username_str = f"@{user_data['username']}" if user_data.get('username') else "No username"
            full_name = f"{user_data.get('first_name', '')} {user_data.get('last_name', '')}".strip() or "No name"
...
Click below to approve or reject this request."""
            ...
            logger.info(f"📨 Admin notified about request from user {user_id}")
            
        except Exception as e:
            logger.error(f"❌ Error notifying admin: {e}")
    
    async def notify_user_approval_decision(self, user_id: int, approved: bool, reason: str = None):
...
🔥 Welcome to TaskTreasure! 🔥"""
            else:
                reason_text = f"\n📝 **Reason:** {reason}" if reason else ""
                message = f"""❌ **Access Rejected**

Sorry, your access request has been rejected.{reason_text}

⏰ **Next Steps:**
• You can request access again after 3 hours
• Contact @tasktreasur_support if you have questions

📞 **Support:** @tasktreasur_support"""
            ...
            logger.info(f"📨 User {user_id} notified about {'approval' if approved else 'rejection'}")
...
<<<<<<< HEAD
    async def admin_approve_user(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if self.supabase:
                result = self.supabase.table('user_approvals').update({
                    'status': 'approved',
                    'approved_at': datetime.now().isoformat(),
                    'approved_by': user_id,
                    'updated_at': datetime.now().isoformat()
                ...
                if result.data:
                    ...
                    # Notify the user
                    await self.notify_user_approval_decision(target_user_id, True)
                else:
                    await update.message.reply_text(f"❌ User {target_user_id} not found in requests.")
    ...
    async def admin_reject_user(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Admin command to reject a user: /reject <user_id> [reason]"""
        user_id = update.effective_user.id
        ...
        if not context.args:
            await update.message.reply_text(
                ...
                "Example: `/reject 123456789 Spam account`"
            )
            return
        ...
        try:
            ...
            if self.supabase:
                ...
                if result.data:
                    ...
                    await update.message.reply_text(f"❌ User {target_user_id} has been rejected{reason_text}")
                    # Notify the user
                    await self.notify_user_approval_decision(target_user_id, False, reason)
                else:
                    await update.message.reply_text(f"❌ User {target_user_id} not found in requests.")
    ...
    async def admin_add_user(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Admin command to directly add a user: /adduser <user_id>"""
        user_id = update.effective_user.id
        ...
        try:
            ...
            # Insert or update user as approved
            if self.supabase:
                data = {
                ...
                result = self.supabase.table('user_approvals').upsert(data, on_conflict='user_id').execute()
                ...
                if result.data:
                    ...
                    await update.message.reply_text(f"❌ Failed to add user {target_user_id}.")
    ...
    async def admin_remove_user(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Admin command to remove a user: /removeuser <user_id>"""
        user_id = update.effective_user.id
    ...
    async def admin_pending_requests(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Admin command to show pending requests: /pending"""
        user_id = update.effective_user.id
        ...
        try:
            if self.supabase:
                result = self.supabase.table('user_approvals').select('*').eq('status', 'pending').order('requested_at', desc=False).execute()
                ...
                for i, request in enumerate(result.data[:10], 1):  # Show max 10
                    user_info = f"{request.get('first_name', 'No name')}"
                    username = f"@{request['username']}" if request.get('username') else "No username"
                    requested_time = datetime.fromisoformat(request['requested_at'].replace('Z', '+00:00')).strftime('%m-%d %H:%M')
                    
                    message += f"{i}. **{user_info}** ({username})\n"
                    message += f"   ID: `{request['user_id']}`\n"
                    message += f"   Requested: {requested_time}\n\n"
                
                message += "💡 **Commands:**\n"
                message += "• `/approve <user_id>` - Approve user\n"
                message += "• `/reject <user_id> [reason]` - Reject user"
...
<<<<<<< HEAD
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not self.is_admin(user_id) and not await self.is_user_approved(user_id):
            user_data = {
                'username': update.effective_user.username,
                'first_name': update.effective_user.first_name,
                'last_name': update.effective_user.last_name
        ...
        if query.data.startswith("approve_"):
            ...
            try:
                ...
                if self.supabase:
                    ...
                    if result.data:
                        await query.edit_message_text(f"✅ **User Approved**\n\nUser {target_user_id} has been approved and notified!")
                        # Notify the user
                        await self.notify_user_approval_decision(target_user_id, True)
                    else:
                        await query.edit_message_text(f"❌ User {target_user_id} not found in requests.")
            except Exception as e:
                await query.edit_message_text(f"❌ Error approving user: {e}")
            ...
            try:
                ...
                if self.supabase:
                    ...
                    if result.data:
                        await query.edit_message_text(f"❌ **User Rejected**\n\nUser {target_user_id} has been rejected and notified.")
                        # Notify the user
                        await self.notify_user_approval_decision(target_user_id, False, "Rejected by admin")
                    else:
                        await query.edit_message_text(f"❌ User {target_user_id} not found in requests.")
            except Exception as e:
                await query.edit_message_text(f"❌ Error rejecting user: {e}")
        
        # Regular callback queries (only for approved users)
